...
class DataDict(dict):
  ...
  def theo_equation(self):
    ...
    if poly_ok:
      for date in dates:
        self[date].theo_equ = poly_theo(date.timetuple().tm_yday)
...
def plot_data(ax_conso, ax_theo, xcoord, dates,
  ...
  ax_theo.plot(
    xcoord, theo_uses, line_style, color="firebrick",
    linewidth=1, markersize=8,
    solid_capstyle="round", solid_joinstyle="round",
    label="conso\nthéorique (%)"
  )
  ax_theo.plot(
    xcoord, real_uses, line_style, color="forestgreen",
    linewidth=1, markersize=8,
    solid_capstyle="round", solid_joinstyle="round",
    label="conso\nréelle (%)"
  )
  ...
  x = np.array(xcoord[min_coord:])
  y = np.array(real_uses[min_coord:])
  ...
  poly_real = np.poly1d(np.linalg.lstsq(A, y)[0])
  ax_theo.plot(
    xcoord, poly_real(xcoord), "--",
...
if __name__ == '__main__':
  ...
  OUT["STATS"] = "stat.dat"
  file_stat = os.path.join(DIR["DATA"], OUT["STATS"])
  ...
  plot_config(
    fig, ax_conso, ax_theo, xcoord, dates, title, conso_per_day
  )
