...
def string_to_date(ssaammjj, fmt="%Y-%m-%d"):
    """
    """
    # print('string_to_date : ' + str(ssaammjj), '-- Type : ' + str(type(ssaammjj)))
...
class Project(object):
    ...
    # ---------------------------------------
    def load_data_utheo(self, filein):
        """
        Load the data from the OUT_CONSO_UTHEO file.
        Created to solve type errors with loading done with np.genfromtxt() directly used in get_date_init.
        Used by get_date_init().

        :param filein: path or name of the file where to get the data (generated by get_input_files)
        :type filein: (unicode) str
        :return date: tuples of datetime corresponding to the date data in utheo
        :rtype date: tuple
        :return utheo: tuples of datetime corresponding to the percentage_of_theoretical_consumption data in utheo
        :rtype utheo: tuple
        """
        dates = ()
        utheos = ()
        with open(filein, "r") as file:
            for ligne in file:
                # skip the header
                if "date" in ligne or ligne.strip() == '':
                    continue
                # read and store the data
                else:
                    splitted_ligne = ligne.strip().split(' ')
                    date = string_to_date(splitted_ligne[0])
                    # check if we actualy have data
                    if not splitted_ligne[-1] == 'nan':
                        percent = string_to_percent(splitted_ligne[-1])
                        # write correct data to the tuples
                        dates = dates + (date,)
                        utheos = utheos + (percent,)
    ...
    def get_date_init(self, filein):
        ...
        dates, utheos = self.load_data_utheo(filein)
