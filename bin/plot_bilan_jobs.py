...
def plot_init():
  ...
  fig, (ax_conso, ax_jobs) = plt.subplots(
    nrows=2,
    ncols=1,
    sharex=True,
    squeeze=True,
  ...
  return fig, ax_conso, ax_theo, ax_jobs


########################################
def plot_data(ax_conso, ax_theo, ax_jobs, xcoord, dates,
  ...
  ax_jobs.bar(
    xcoord, run_mean, width=width, align="center",
    # yerr=run_std/2, ecolor="green",
    color="lightgreen", linewidth=line_width,
    antialiased=True, label="jobs running"
  )
  ax_jobs.bar(
    xcoord, pen_mean, bottom=run_mean, width=width, align="center",
    # yerr=pen_std/2, ecolor="darkred",
...
def plot_config(fig, ax_conso, ax_theo, xcoord, dates, title,
  ...
  ax_jobs.set_xticks(xcoord, minor=True)
  ax_jobs.set_xticks(maj_xticks, minor=False)
...
if __name__ == '__main__':
  ...
  # ... Initialize figure ...
  # -------------------------
  (fig, ax_conso, ax_theo, ax_jobs) = plot_init()
  ...
  # ... Tweak figure ...
  # --------------------
  title = "{} : Conso globale et suivi des jobs " \
          "(moyenne journalière)\n({:%d/%m/%Y} - {:%d/%m/%Y})".format(
